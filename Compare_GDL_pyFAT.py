...
def main():
    ...
    directory = '/media/peter/Data/FAT_Main/Analysis/Compare_GDL_pyFAT/'
    create_individual = ['pyFAT','GDL']
    #create_individual=['GDL']
    ...
    if read_all_input:
        ...
        with open(f"{directory}Input_Parameters_GDL{Gversion}_pyFAT{Pversion}.pkl",'wb')\
              as tmp:
            pickle.dump(Input_Parameters,tmp)
    else:
        with open(f'{directory}Input_Parameters_GDL{Gversion}_pyFAT{Pversion}.pkl', 'rb') as f:
            Input_Parameters = pickle.load(f)
    #TMP_par ={}
    #TMP_par['HPASS00000'] = Input_Parameters['HPASS00000']
    #TMP_par['Circinus_18.9Beams_1.0SNR'] = Input_Parameters['Circinus_18.9Beams_1.0SNR']
    #print(f'Lets print TMP par it should have 2 galaxies')
    #print(TMP_par)
    #exit()
    if adddelt:
        sf.add_deltas(Input_Parameters)
        with open(f'{directory}Input_Parameters_delt_GDL{Gversion}_pyFAT{Pversion}.pkl','wb')\
              as tmp:
            pickle.dump(Input_Parameters,tmp)
    else:
        with open(f'{directory}Input_Parameters_delt_GDL{Gversion}_pyFAT{Pversion}.pkl', 'rb') as f:
            Input_Parameters = pickle.load(f)
        ...
        for program in create_individual:
            for database in Input_File[program]:
                if database in ['version','program']:
                    continue
                print(f'Starting to analyz the database {database} fitted with {program} individually')
