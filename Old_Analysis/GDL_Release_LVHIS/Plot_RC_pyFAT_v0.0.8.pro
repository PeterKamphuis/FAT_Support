...
main_dir='/home/peter/FAT/LVHIS-26_pyFAT_v0.0.8/'

catname=main_dir+'pyFAT_results.txt'
...
;   print,HPASSName[tmp],values[1],values[2]
   case 1 OF
      ...
      values[1] EQ 'False': fitresult[tmp] = 1.5
   ENDCASE
ENDWHILE
close,1
...
for i=0,n_elements(HPASSNAme)-1 do begin
  ...
  ;    XYOUTS,0.1+(xnum+1)*ixsize-0.025,0.1+(ynum)*iysize+0.8*iysize,alphabet[n_elements(HPASSNAme)-1-acount],/normal,alignment=1.0
      ...
      XYOUTS,position[2]-0.01,position[1]+0.75*iysize,alphabet[n_elements(HPASSNAme)-1-acount],/normal,alignment=1.0
      plotsym, 3 ,/fill
...
;XYOUTS,position[0]+0.01,position[1]+0.8*iysize,'Star',/normal,alignment=1.0
   ...
   ENDIF ELSE BEGIN
      print,'We are skipping '+JName[i]+' '+JNumber[i]
      print,TOTAL(Tirific[*,5,i]),fitresult[i]
      stop
   ENDELSE 
ENDfor
